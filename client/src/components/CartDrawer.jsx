    ...
    <Fragment>
      ...
      >
        <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
          <h3 className="text-lg font-bold text-gray-900 dark:text-white">Tu carrito</h3>
          <button onClick={closeCart} className="p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-700">
              ...
              <div key={item.product._id} className="flex gap-3 items-center">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start">
                    <h4 className="text-sm font-medium text-gray-900 dark:text-white truncate">{item.product.name}</h4>
                    <button onClick={() => removeFromCart(item.product._id)} className="text-red-500 hover:text-red-700 p-1 rounded">
                      <Trash2 className="w-4 h-4" />
                    </button>
                  </div>
                  <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">{new Intl.NumberFormat("es-ES", { style: "currency", currency: "EUR" }).format(item.product.price)}</p>
        ...
        <div className="p-4 border-t border-gray-200 dark:border-gray-700">
          <div className="flex justify-between items-baseline mb-3">
            ...
            <span className="text-lg font-bold text-primary-500">{new Intl.NumberFormat("es-ES", { style: "currency", currency: "EUR" }).format(totalPrice)}</span>
          </div>
          ...
          <div className="space-y-2">
            ...
            <button onClick={clearCart} className="w-full px-4 py-2 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-full font-medium transition-colors text-sm">
              Vaciar carrito
            </button>
          </div>
        </div>
