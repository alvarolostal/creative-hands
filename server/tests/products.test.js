...
test('non-admin cannot create product (403)', async () => {
  ...
  const res = await request(app).post('/api/products').set('Authorization', `Bearer ${token}`).send(productPayload);
  expect(res.statusCode).toBe(403);
});

test('admin can update and delete product', async () => {
  const admin = await User.create({ name: 'Admin2', email: 'admin2@test.com', password: 'Admin123!', role: 'admin' });
  const token = jwt.sign({ id: admin._id }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPIRE });

  const productPayload = { name: 'ToUpdate', description: 'x', price: 5 };
  const createRes = await request(app).post('/api/products').set('Authorization', `Bearer ${token}`).send(productPayload);
  expect(createRes.statusCode).toBe(201);
  const prodId = createRes.body.product._id;

  // update
  const updateRes = await request(app).put(`/api/products/${prodId}`).set('Authorization', `Bearer ${token}`).send({ name: 'Updated' });
  expect(updateRes.statusCode).toBe(200);
  expect(updateRes.body.product.name).toBe('Updated');

  // delete
  const delRes = await request(app).delete(`/api/products/${prodId}`).set('Authorization', `Bearer ${token}`);
  expect(delRes.statusCode).toBe(200);

  // ensure not present
  const listRes = await request(app).get('/api/products');
  const found = listRes.body.products.find(p => p._id === prodId);
  expect(found).toBeUndefined();
});

test('GET /api/products is public and returns products', async () => {
  // create admin + product
  const admin = await User.create({ name: 'Admin3', email: 'admin3@test.com', password: 'Admin123!', role: 'admin' });
  const token = jwt.sign({ id: admin._id }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPIRE });
  const productPayload = { name: 'PublicProduct', description: 'pub', price: 3 };
  await request(app).post('/api/products').set('Authorization', `Bearer ${token}`).send(productPayload);
